...
'''System exit'''
def system_exit(error):
    global _status
    write_log_entry(_status, error, 'ERROR') 
    ...
    def run(self):
        ...
        while not self.stopped.wait(self.period):
            ...
            for result in files_to_cloud:
                ...
                update_file_info_query = f'''
                UPDATE {app_config.db_table_main_name} 
                SET "InColdStorage" = True 
                WHERE "FileID" = {file_id}
                '''
                psql_connector.query_execute(update_file_info_query)
